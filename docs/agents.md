...
|---|---|---|
| Orchestrator | primary | User-facing coordinator and state-machine router; clarifies requests, applies docs-first routing to documentation when durable context is needed, owns high-level task decomposition, writes planning handoff artifacts directly for non-trivial work, provides a structured handoff to task-planner, delegates to subagents, and synthesizes results |
| Executor | subagent | Performs exact, mechanical, single-file atomic edits that do not need a task spec, tests, or validation |
| Task Planner | subagent | Creates auditable task specs under `.ai/tasks/` from an orchestrator handoff; reads planning handoff file when provided and treats it as canonical source context; owns path allocation, required spec sections, parent/child manifest mechanics, and planning-blocked behavior; formalizes plans without inventing missing strategic decisions; accepts assigned output paths for unit-level planning |
| Implementer | subagent | Edits source code according to an approved task spec and writes an implementation report; uses file-based agy handoff (agy-handoff.md) with intentional bounded backend skip-permissions mode when agy is enabled |
...
- **Orchestrator**: `edit: deny` with a single narrow exception for `.ai/tasks/**/planning-handoff.md` — the orchestrator may write planning handoff artifacts directly, but cannot edit source code, docs, configuration, task specs, reports, or any other file. Its `bash` permission is a strict whitelist limited to read-only informational commands (`ls`, `pwd`, `which`, `env`, `echo`, `uname`, `file`, `wc`, `git status`, `git diff`, `git log`, `git branch`, `git remote`, `git rev-parse`, `git show`, `git config`, `git stash list`, `git ls-files`). All other shell commands (`*`) are denied. Git write commands (`git commit*`, `git push*`) are explicitly denied.
...
- **Implementer**: `bash: * allow` so it can seamlessly run test, build, and dev commands without permission prompts. Its `edit` scope prevents it from touching `.ai/tasks/**` (except its own `implementation-report.md` and `agy-handoff.md`), `.ai/context.md`, and `.ai/decisions/**` — it can edit source code freely but cannot alter task artifacts. The agy integration uses `--dangerously-skip-permissions` in bounded backend mode: the implementer writes a complete handoff file before invoking agy, and the flag is safe because the work scope is fully bounded by the inspected handoff document.
